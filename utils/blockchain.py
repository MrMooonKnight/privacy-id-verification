...
def load_contract_abi(contract_name):
    ...
    try:
        # Try the blockchain build directory first
        try:
        ...
        # Try the contracts directory in root
        try:
            with open(f'contracts/{contract_name}.json', 'r') as file:
                contract_json = json.load(file)
                return contract_json['abi'] if 'abi' in contract_json else contract_json
        except FileNotFoundError:
            pass
        ...
        contract_paths = [
            os.path.join(root_dir, f'contracts/{contract_name}.json'),
            os.path.join(root_dir, f'blockchain/contracts/{contract_name}.json'),
            os.path.join(root_dir, f'blockchain/build/contracts/{contract_name}.json'),
            os.path.join(current_dir, f'../contracts/{contract_name}.json')
        ]
        ...
        raise FileNotFoundError(f"Could not find ABI for contract {contract_name}. Searched paths: {contract_paths}")
        
    except Exception as e:
        print(f"Error loading contract ABI: {str(e)}")
        # For development/testing, return a minimal ABI
        print("Using fallback minimal ABI for development")
        return [
            {
                "inputs": [
                    {"internalType": "bytes", "name": "userId", "type": "bytes"},
                    {"internalType": "bytes", "name": "identityHash", "type": "bytes"}
                ],
            ...
            {
                "inputs": [{"internalType": "bytes", "name": "userId", "type": "bytes"}],
